...
def get_clips_from_gemini(transcript_text, gemini_model_name, logger_func=print):
    ...
    ATURAN UTAMA:
    1. HOOK ADALAH SEGALANYA: Setiap klip yang Anda sarankan HARUS dimulai dengan hook yang sangat kuat. Jika segmen tidak memiliki hook, JANGAN JADIKAN KLIP.
    ...
    4. OUTPUT JSON: Harus berupa format JSON valid `[ ... ]`.
    <transcript>{transcript_text}</transcript>
    INSTRUKSI KLIP:
    1. Cari Hook: Pertanyaan, pernyataan kontroversial, momen emosional, atau klimaks.
    2. Tentukan Waktu: "start_time" harus TEPAT DI AWAL HOOK.
    3. Metadata: Buat "title" clickbait, 3 "hashtags", dan "editing_style".
    """
    try:
        ...
        logger_func(f"❌ ERROR saat analisis AI (klip): {e}"); return []
...
def get_paraphrased_title_from_gemini(original_title, gemini_model_name, logger_func=print):
    ...
    try:
        ...
        logger_func(f"   ❌ Gagal membuat judul dengan AI: {e}")
        return None

def process_clip(source_video, start_time, end_time, watermark_file, source_text, output_filename, style, music_file, music_volume, word_segments, effects, remove_original_audio, original_audio_volume, is_short_mode=False, logger_func=print):
    try:
        main_video = ffmpeg.input(source_video, ss=start_time, to=end_time)
        watermark = ffmpeg.input(watermark_file)
        
        if is_short_mode:
            # --- MODIFIKASI: CROP DIBUAT LEBIH JELAS (90% DARI FRAME ASLI) ---
            # Ini akan mengambil 90% area tengah video, memberikan efek zoom yang lebih terlihat.
            processed_video = main_video.video.filter('crop', 'in_w*0.90', 'in_h*0.90')
        else:
        ...
        if is_short_mode:
            ...
            if final_audio:
                final_audio = final_audio.filter('atempo', speed)
...
class VideoClipperApp:
    def __init__(self, root):
        ...
        self.use_ai_for_shorts_title = BooleanVar(value=True)
    ...
    def setup_ui(self):
        ...
        middle_frame = Frame(main_frame); middle_frame.pack(fill="both", expand=True, pady=5)
        left_column = Frame(middle_frame, width=400); left_column.pack(side="left", fill="y", padx=(0, 10)); left_column.pack_propagate(False)
        right_column = Frame(middle_frame); right_column.pack(side="left", fill="both", expand=True)
        ...
        id_entry = Entry(license_lf, textvariable=self.device_id_var, state="readonly", width=30); id_entry.grid(row=0, column=0, columnspan=2, sticky="ew")
        ...
        scraper_lf = LabelFrame(left_column, text="Mode Scraper Shorts", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        scraper_lf.pack(fill="x", pady=(0,10))
        ...
        self.scrape_button = Button(count_frame, text="🔎 Cari & Tempel Link", command=self.start_scraping_thread)
        self.scrape_button.pack(side="left", expand=True, fill="x")

        settings_lf = LabelFrame(left_column, text="Pengaturan Editing Video", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        ...
        cut_mode_lf = LabelFrame(left_column, text="Mode Pemotongan Video (Non-Shorts)", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        cut_mode_lf.pack(fill="x", pady=(0, 10))
        model_frame = Frame(cut_mode_lf); model_frame.pack(fill='x', pady=5, anchor="w")
        Label(model_frame, text="Akurasi AI:").pack(side="left", padx=(0,10))
        ...
        self.start_button = Button(action_lf, text="🚀 Mulai Proses Video", command=self.start_processing_thread, bg="#28a745", fg="white", font=("Helvetica", 12, "bold"), relief="raised"); self.start_button.pack(fill="x", ipady=8)
    ...
    def start_scraping_thread(self):
        url = self.scrape_channel_url.get()
        if not url: messagebox.showerror("Error", "Masukkan URL Channel YouTube terlebih dahulu."); return
        self.scrape_button.config(state="disabled", text="Mencari...")
        threading.Thread(target=self.scrape_shorts_from_channel, daemon=True).start()

    def scrape_shorts_from_channel(self):
        channel_url = self.scrape_channel_url.get()
        count = self.scrape_count.get()
    ...
    def toggle_manual_cut_fields(self):
        if self.cut_mode.get() == "manual": self.start_entry.config(state="normal"); self.end_entry.config(state="normal")
        else: self.start_entry.config(state="disabled"); self.end_entry.config(state="disabled")
    ...
    def get_and_copy_uuid(self):
        try: device_id = machineid.id(); self.device_id_var.set(device_id); self.root.clipboard_clear(); self.root.clipboard_append(device_id); messagebox.showinfo("ID Disalin", "ID Perangkat Anda telah disalin ke clipboard.")
    ...
    def select_music(self):
        ...
        if file: self.music_full_path = file; self.music_file.set(f"Musik: {os.path.basename(file)}")
    ...
    def run_processing_logic(self):
        ...
        total_steps = 4 if is_shorts_mode else 7
        try:
            ...
            if is_shorts_mode:
                ...
                for index, video_url in enumerate(video_urls):
                    ...
                    self.update_progress(3, total_steps, f"Mengunduh & Memproses Short ({index+1}/{len(video_urls)})...")
                    video_path, info = download_video(video_url, logger_func=self.log)
                    ...
                    if self.use_ai_for_shorts_title.get():
                        self.log(f"   🤖 Meminta AI untuk membuat judul baru...")
                        new_title = get_paraphrased_title_from_gemini(original_title, GEMINI_MODEL, self.log)
                        if new_title:
                            final_title = new_title
                            self.log(f"   ✅ Judul baru dari AI: {final_title}")
                        else:
                            self.log(f"   ⚠️ Gagal mendapatkan judul dari AI, menggunakan judul asli.")
                    ...
                    music_volume, original_volume, remove_original = self.music_volume_var.get(), self.original_audio_volume_var.get(), self.remove_original_audio_var.get()
                    
                    process_clip(
                        source_video=video_path, start_time="00:00:00", end_time=time.strftime('%H:%M:%S', time.gmtime(info.get('duration', 60))),
                        watermark_file=self.watermark_full_path, source_text="", output_filename=output_file, 
                        style='informative', music_file=self.music_full_path, music_volume=music_volume,
                        word_segments=[], effects=selected_effects, remove_original_audio=remove_original,
                        original_audio_volume=original_volume, is_short_mode=True, logger_func=self.log
                    )
                    os.remove(video_path)
                    self.log(f"   🗑️ File short asli ({os.path.basename(video_path)}) telah dihapus.")
                self.update_progress(4, total_steps, "Semua proses Shorts selesai!")

            else:
                self.log("\n🚀 MENJALANKAN MODE PEMOTONGAN VIDEO 🚀")
                self.update_progress(3, total_steps, "Memuat Model Transkripsi...")
                ...
                for index, video_url in enumerate(video_urls):
                    ...
                    self.update_progress(4, total_steps, f"Mengunduh Video ({index+1}/{len(video_urls)})...")
                    video_path, info = download_video(video_url, logger_func=self.log)
                    if not video_path: continue
                    channel_name = info.get('uploader', 'Unknown Channel')

                    self.update_progress(5, total_steps, f"Transkripsi Audio ({index+1}/{len(video_urls)})...")
                    ...
                    if self.cut_mode.get() == "otomatis":
                        self.update_progress(6, total_steps, f"Mencari Klip Viral dengan AI ({index+1}/{len(video_urls)})...")
                        ai_clips = get_clips_from_gemini(transcription_result['text'], GEMINI_MODEL, self.log)
                    else: 
                        self.update_progress(6, total_steps, f"Menggunakan Waktu Manual ({index+1}/{len(video_urls)})...")
                    ...
                    if not ai_clips: self.log("   🔴 Tidak ada klip untuk diproses."); continue
                    self.update_progress(7, total_steps, f"Membuat Klip Video ({index+1}/{len(video_urls)})...")
                    ...
                    for i, clip in enumerate(ai_clips):
                        ...
                        clip_words = [word for s in transcription_result.get('segments',[]) for word in s.get('words',[]) if word['start'] < sum(x * float(t) for x, t in zip([3600, 60, 1], end_time_str.split(":"))) and word['end'] > sum(x * float(t) for x, t in zip([3600, 60, 1], start_time_str.split(":")))]
                        if not clip_words and transcription_result['text']: clip_words.append({'word': ' ', 'start': 0, 'end': 1})
                        if not clip_words: continue
                        safe_filename = sanitize_filename(f"{clip.get('title', f'Klip {i+1}')} {' '.join(clip.get('hashtags', []))}")
                        output_file = os.path.join(output_folder, f"{safe_filename}.mp4")
                        process_clip(source_video=video_path, start_time=start_time_str, end_time=end_time_str, watermark_file=self.watermark_full_path, source_text=f"Sumber: {channel_name}", output_filename=output_file, style=clip.get('editing_style', 'informative'), music_file=self.music_full_path, music_volume=music_volume, word_segments=clip_words, effects=selected_effects, remove_original_audio=remove_original, original_audio_volume=original_volume, is_short_mode=False, logger_func=self.log)
                    os.remove(video_path)
                    self.log(f"   🗑️ File video asli ({os.path.basename(video_path)}) telah dihapus.")
                self.update_progress(7, total_steps, "Semua proses video selesai!")
