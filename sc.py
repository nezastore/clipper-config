...
import queue
import textwrap
from tkinter import Tk, filedialog, Button, Label, Text, Scrollbar, Frame, messagebox, StringVar, OptionMenu, Entry, Checkbutton, BooleanVar, Scale, IntVar
...
# Fungsi-fungsi utility (sanitize_filename, dll.)
...
def process_clip(source_video, start_time, end_time, watermark_file, source_text, output_filename, style, music_file, music_volume, word_segments, effects, logger_func=print):
    try:
        ...
        if music_file:
            ...
            # Menggunakan nilai volume dari slider
...
class VideoClipperApp:
    def __init__(self, root):
        ...
        # Variabel baru untuk slider volume
        self.music_volume_var = IntVar(value=20) # Default 20%
        self.volume_display_var = StringVar(value="20%") # Untuk label di samping slider
    ...
    def setup_ui(self):
        ...
        license_frame = Frame(main_frame, bd=2, relief="groove", padx=10, pady=10)
        license_frame.pack(fill="x", pady=(0, 10))
        Label(license_frame, text="Manajemen Lisensi", font=("Helvetica", 11, "bold")).grid(row=0, column=0, columnspan=3, sticky="w", pady=(0,10))
        id_entry = Entry(license_frame, textvariable=self.device_id_var, state="readonly", width=40)
        id_entry.grid(row=1, column=0, columnspan=2, sticky="ew")
        Button(license_frame, text="Salin ID Perangkat", command=self.get_and_copy_uuid).grid(row=1, column=2, padx=(10,0))
        Label(license_frame, text="Status Lisensi:").grid(row=2, column=0, sticky="w", pady=(5,0))
        self.license_status_label = Label(license_frame, text="- MENGECEK -", font=("Helvetica", 10, "bold"), fg="grey")
        self.license_status_label.grid(row=2, column=1, sticky="w", pady=(5,0))
        license_frame.columnconfigure(1, weight=1)

        input_frame = Frame(main_frame, bd=2, relief="groove", padx=10, pady=10)
        input_frame.pack(fill="x", pady=5)
        Label(input_frame, text="1. Pengaturan File & Model AI", font=("Helvetica", 11, "bold")).pack(anchor="w")
        Button(input_frame, text="Pilih Folder Simpan Video", command=self.select_output_folder).pack(fill="x", pady=5)
        Label(input_frame, textvariable=self.output_folder, fg="blue").pack(anchor="w")
        Button(input_frame, text="Pilih File Watermark (Gambar)", command=self.select_watermark).pack(fill="x", pady=(10,5))
        Label(input_frame, textvariable=self.watermark_file, fg="blue").pack(anchor="w")
        Button(input_frame, text="Pilih Musik Latar (Opsional)", command=self.select_music).pack(fill="x", pady=(10,5))
        Label(input_frame, textvariable=self.music_file, fg="blue").pack(anchor="w")

        # --- SLIDER VOLUME BARU ---
        volume_frame = Frame(input_frame)
        volume_frame.pack(fill="x", padx=5, pady=(5, 10))
        Label(volume_frame, text="Volume Musik Latar:").pack(side="left")
        volume_slider = Scale(volume_frame, from_=0, to=100, orient="horizontal", variable=self.music_volume_var, command=self.update_volume_label, length=200)
        volume_slider.pack(side="left", expand=True, fill="x", padx=10)
        Label(volume_frame, textvariable=self.volume_display_var, width=5).pack(side="left")

        model_frame = Frame(input_frame)
        model_frame.pack(fill='x', pady=(15,5))
        Label(model_frame, text="Pilih Model Akurasi Subtitle:").pack(side="left", padx=(0,10))
        OptionMenu(model_frame, self.whisper_model_selection, *["base", "small", "medium"]).pack(side="left")

        effects_frame = Frame(main_frame, bd=2, relief="groove", padx=10, pady=10)
        effects_frame.pack(fill="x", pady=5)
        Label(effects_frame, text="2. Pengaturan Efek Video (Opsional)", font=("Helvetica", 11, "bold")).pack(anchor="w")
        Checkbutton(effects_frame, text="Mirror (Cermin Horizontal)", variable=self.effects_vars['mirror']).pack(anchor="w")
        Checkbutton(effects_frame, text="Grayscale (Hitam Putih)", variable=self.effects_vars['grayscale']).pack(anchor="w")
        Checkbutton(effects_frame, text="Sepia", variable=self.effects_vars['sepia']).pack(anchor="w")
        Checkbutton(effects_frame, text="Negate (Warna Negatif)", variable=self.effects_vars['negate']).pack(anchor="w")
        Checkbutton(effects_frame, text="Color Boost (Saturasi Tinggi)", variable=self.effects_vars['color_boost']).pack(anchor="w")

        url_frame = Frame(main_frame, bd=2, relief="groove", padx=10, pady=10)
        url_frame.pack(fill="both", pady=5, expand=True)
        Label(url_frame, text="3. Masukkan Link Video", font=("Helvetica", 11, "bold")).pack(anchor="w")
        self.url_text = Text(url_frame, height=5, relief="solid", borderwidth=1)
        self.url_text.pack(fill="both", expand=True, pady=5)
        
        action_frame = Frame(main_frame)
        action_frame.pack(fill="both", expand=True)
        self.start_button = Button(action_frame, text="🚀 Mulai Proses Video", command=self.start_processing_thread, bg="#28a745", fg="white", font=("Helvetica", 12, "bold"), relief="raised")
        self.start_button.pack(fill="x", ipady=8, pady=10)
        self.progress_bar = Progressbar(action_frame, orient="horizontal", length=100, mode="determinate")
        self.progress_bar.pack(fill="x", pady=(0, 5))
        log_frame = Frame(action_frame, bd=2, relief="groove")
        log_frame.pack(fill="both", expand=True)
        Label(log_frame, text="Log Proses:", font=("Helvetica", 11, "bold")).pack(anchor="w", padx=5, pady=2)
        self.log_text = Text(log_frame, state='disabled', wrap='word', relief="flat", height=10)
    ...
    def update_volume_label(self, value):
        self.volume_display_var.set(f"{int(float(value))}%")
    ...
    def get_and_copy_uuid(self):
        try:
            ...
            messagebox.showerror("Error", f"Gagal mendapatkan ID Perangkat: {e}")
