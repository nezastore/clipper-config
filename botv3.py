...
def process_clip(source_video, start_time, end_time, watermark_file, watermark_position, source_text, output_filename, style, music_file, music_volume, word_segments, effects, remove_original_audio, original_audio_volume, is_short_mode=False, logger_func=print):
    try:
        ...
        if is_short_mode:
            ...
            # Cek jika video sudah vertikal (9:16)
            if abs((w / h) - (9 / 16)) < 0.01:
                 # Jika sudah, tidak perlu crop
                processed_video = main_video.video
            else:
                # Jika tidak, crop ke 9:16
                target_h = h
        ...
        if watermark_file:
            ...
            pos_map = {
                "Kanan Atas": {'x': 'main_w-overlay_w-10', 'y': '10'},
                "Kiri Atas": {'x': '10', 'y': '10'},
                "Kanan Bawah": {'x': 'main_w-overlay_w-10', 'y': 'main_h-overlay_h-10'},
                "Kiri Bawah": {'x': '10', 'y': 'main_h-overlay_h-10'},
                "Tengah": {'x': '(main_w-overlay_w)/2', 'y': '(main_h-overlay_h)/2'}
            }
    ...
    # --- BLOK PENANGANAN ERROR ---
        ...
        logger_func("--- Detail Error dari FFMPEG ---")
...
class VideoClipperApp:
    ...
    def setup_ui(self):
        ...
        # --- WADAH BARU UNTUK MENGATUR POSISI KONTEN UTAMA ---
        content_wrapper = Frame(main_frame)
        content_wrapper.pack(fill="both", expand=True) # expand=True agar mengisi frame utama
        
        # --- KONTEN UTAMA (KIRI DAN KANAN) DI DALAM FRAME YANG TIDAK MEREGANG ---
        main_content_frame = Frame(content_wrapper)
        main_content_frame.pack() # Defaultnya akan berada di tengah

        # --- KOLOM KIRI DENGAN SCROLLBAR ---
        left_container = Frame(main_content_frame, width=420)
        ...
        # --- KOLOM KANAN ---
        right_column = Frame(main_content_frame)
        right_column.pack(side="left", fill="y") 

        # --- KONTEN KOLOM KIRI (DI DALAM scrollable_frame) ---
        ...
        api_lf = LabelFrame(scrollable_frame, text="Konfigurasi API Key", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        api_lf.pack(fill="x", pady=(0,10), padx=10)
        ...
        scraper_lf = LabelFrame(scrollable_frame, text="Mode Scraper Shorts", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        scraper_lf.pack(fill="x", pady=(0,10), padx=10)
        ...
        file_lf = LabelFrame(scrollable_frame, text="File & Aset", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        file_lf.pack(fill="x", pady=(0,10), padx=10)
        ...
        Button(file_lf, text="Pilih Gambar Thumbnail (9:16)", command=self.select_thumbnail).pack(fill="x")
        Label(file_lf, textvariable=self.thumbnail_file, fg="blue", wraplength=350).pack(anchor="w", padx=2, pady=(0,5))
        Checkbutton(file_lf, text="Aktifkan Thumbnail Kustom", variable=self.use_custom_thumbnail).pack(anchor="w")

        audio_lf = LabelFrame(scrollable_frame, text="Pengaturan Audio", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        audio_lf.pack(fill="x", pady=(0,10), padx=10)
        ...
        self.original_audio_slider = Scale(original_volume_frame, from_=0, to=100, orient="horizontal", variable=self.original_audio_volume_var, command=self.update_original_volume_label)
        self.original_audio_slider.pack(side="left", expand=True, fill="x", padx=5)
        ...
        self.music_slider = Scale(music_volume_frame, from_=0, to=100, orient="horizontal", variable=self.music_volume_var, command=self.update_music_volume_label, state="disabled")
        self.music_slider.pack(side="left", expand=True, fill="x", padx=5)
        Label(music_volume_frame, textvariable=self.volume_display_var, width=4).pack(side="left")

        # --- KONTEN KOLOM KANAN ---
        cut_mode_lf = LabelFrame(right_column, text="Mode Pemotongan Video (Mode Long-Short)", font=("Helvetica", 10, "bold"), padx=10, pady=10)
        ...
        self.toggle_original_audio_slider()
    ...
    def toggle_original_audio_slider(self):
        state = "disabled" if self.remove_original_audio_var.get() else "normal"
        self.original_audio_slider.config(state=state)

    def select_thumbnail(self):
        file = filedialog.askopenfilename(title="Pilih Gambar Thumbnail (9:16)", filetypes=[("Image Files", "*.png;*.jpg;*.jpeg")])
    ...
    def toggle_manual_cut_fields(self):
        state = "normal" if self.cut_mode.get() == "manual" else "disabled"
    ...
    def start_processing_thread(self):
        ...
        if self.use_custom_thumbnail.get() and not self.thumbnail_full_path: messagebox.showerror("Error", "Pilih file gambar untuk thumbnail."); return
    ...
    def run_processing_logic(self):
        ...
        total_steps = 3 if is_shorts_mode else 6 
        if self.use_custom_thumbnail.get(): total_steps += 1
        
        try:
            step = 1
            self.update_progress(step, total_steps, "Memverifikasi Lisensi..."); step+=1
            if self.stop_event.is_set(): return
            is_valid, _ = verify_license(self.log); 
            if not is_valid: self.log("   Silakan hubungi admin Telegram : @nezastore."); return
            
            self.update_progress(step, total_steps, "Memuat Konfigurasi & Model AI..."); step+=1
            if self.stop_event.is_set(): return
            ...
            if self.use_custom_thumbnail.get():
                self.update_progress(step, total_steps, "Mempersiapkan Thumbnail..."); step+=1
            ...
            if is_shorts_mode:
                ...
                for index, video_url in enumerate(video_urls):
                    ...
                    self.update_progress(step, total_steps, f"Mengunduh & Memproses Short ({index+1}/{len(video_urls)})...")
                    ...
                    if self.stop_event.is_set(): os.remove(video_path); return
                    process_clip(source_video=video_path, start_time="00:00:00", end_time=time.strftime('%H:%M:%S', time.gmtime(info.get('duration', 60))), watermark_file=self.watermark_full_path, watermark_position=self.watermark_position.get(), source_text="", output_filename=output_file, style='informative', music_file=self.music_full_path, music_volume=self.music_volume_var.get(), word_segments=[], effects={k: v.get() for k, v in self.effects_vars.items()}, remove_original_audio=self.remove_original_audio_var.get(), original_audio_volume=self.original_audio_volume_var.get(), is_short_mode=True, logger_func=self.log)
                ...
                for index, video_url in enumerate(video_urls):
                    ...
                    self.update_progress(step, total_steps, f"Mengunduh Video ({index+1}/{len(video_urls)})...")
                    video_path, info = download_video(video_url, logger_func=self.log)
                    if not video_path: continue
                    channel_name = info.get('uploader', 'Unknown Channel')

                    if self.stop_event.is_set(): os.remove(video_path); return
                    self.update_progress(step+1, total_steps, f"Transkripsi Audio ({index+1}/{len(video_urls)})...")
                    ...
                    if self.cut_mode.get() == "otomatis":
                        if self.stop_event.is_set(): os.remove(video_path); return
                        self.update_progress(step+2, total_steps, f"Mencari Klip Viral dengan AI ({index+1}/{len(video_urls)})...")
                        ai_clips = get_clips_from_gemini(transcription_result['text'], GEMINI_MODEL, self.log)
                    else: 
                        self.update_progress(step+2, total_steps, f"Menggunakan Waktu Manual ({index+1}/{len(video_urls)})...")
                    ...
                    if self.stop_event.is_set(): os.remove(video_path); return
                    self.update_progress(step+3, total_steps, f"Membuat Klip Video ({index+1}/{len(video_urls)})...")
                    self.log(f"   Ditemukan {len(ai_clips)} potensi klip. Memulai rendering...")
                    
                    for i, clip in enumerate(ai_clips):
                        if self.stop_event.is_set(): os.remove(video_path); return
                        if not isinstance(clip, dict) or not all(k in clip for k in ('start_time', 'end_time')):
                            self.log(f"   ⚠️ Melewati klip ke-{i+1} karena format dari AI tidak valid."); self.log(f"     Data klip dari AI: {clip}"); continue
                        ...
                        output_file = os.path.join(output_folder, f"{safe_filename}.mp4")
                        process_clip(source_video=video_path, start_time=start_time_str, end_time=end_time_str, watermark_file=self.watermark_full_path, watermark_position=self.watermark_position.get(), source_text=f"Sumber: {channel_name}", output_filename=output_file, style=clip.get('editing_style', 'informative'), music_file=self.music_full_path, music_volume=self.music_volume_var.get(), word_segments=clip_words, effects={k: v.get() for k, v in self.effects_vars.items()}, remove_original_audio=self.remove_original_audio_var.get(), original_audio_volume=self.original_audio_volume_var.get(), is_short_mode=False, logger_func=self.log)
                        if self.use_custom_thumbnail.get() and self.thumbnail_full_path:
                            embed_thumbnail(output_file, self.thumbnail_full_path, self.log)
